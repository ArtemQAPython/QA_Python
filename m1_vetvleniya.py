...
#Задание 1
#Напиши код, который определит, верно ли выражение 2 * 2 == 5. Выведи результат на экран.
...
#Ветвление кода
#Задание 1
#Если в пароле 0 символов, нужно вывести на экран «Поле пароля не может быть пустым!». Допиши код проверки.
count_password_symbols = 0
if count_password_symbols == 0:
    print('Поле пароля не может быть пустым!')

#Задание 2
#Если тест не пройден, программа выводит «Тест не пройден!». Допиши код: проверь, чему равно значение переменной.
...
#Задание 2
#Обычно к тестировщику попадают задачи, которые находятся в статусе «Можно тестировать». Напиши условия, которые проверяют, можно ли начинать тестировать задачи или стоит подождать.
#Представь, что всего у тебя три задачи. Их статусы хранятся в переменных: task_status_1 , task_status_2 и task_status_3. Если задачу можно тестировать, нужно вывести «Пора тестировать задачу!». Если нет — «Задача ещё не передана в тестирование!»
...
#Задание 1
#Гости играли в игру, где нужно разбиться на команды. Первая команда выиграла три раза и вторая три раза. Сравни результаты двух команд: пусть программа напишет, кто выиграл в итоге.
#Допиши код, чтобы вывести на экран результат. Используй переменные team1_wins и team2_wins — общее количество побед первой и второй команды. Ещё пригодятся операторы «больше», «меньше» и «равно».
team1_wins = 3
team2_wins = 3
...
#Задание 2
#В прошлой теме тебе уже удалось определить, можно ли начинать тестировать задачу. Можно сделать этот код еще лучше: выводить более подробные статусы.
task_status_1 = 'Можно тестировать'
task_status_2 = 'Открыта'
task_status_3 = 'В работе'
if task_status_1 == 'Можно тестировать':
    print('Пора тестировать задачу!')
elif task_status_1 == 'В работе':
    print('Задача ещё находится в работе')
else:
    print('Разработчик ещё не приступал к работе над задачей')
if task_status_2 == 'Можно тестировать':
    print('Пора тестировать задачу!')
elif task_status_2 == 'В работе':
    print('Задача ещё находится в работе')
else:
    print('Разработчик ещё не приступал к работе над задачей')
if task_status_3 == 'Можно тестировать':
    print('Пора тестировать задачу!')
elif task_status_3 == 'В работе':
    print('Задача ещё находится в работе')
else:
    print('Разработчик ещё не приступал к работе над задачей')

#Задание 3
#Представь, что ты проверяешь кнопки на сайте. Они должны быть разных цветов. В зависимости от цвета кнопка делает разное:
#синий — сохранить;
...
#Логические операции
#Задание 4
#Несколько человек играют в игру, где каждый получает баллы. Каждая переменная playerN означает количество баллов игрока под номером N. Например, player1 — результат первого игрока.
...
#Заполни ..., чтобы код выполнился верно. Используй знаки >, < и оператор or.
player1 = 22
player2 = 23
record = 21
...
#Задание 3
#Осталось заказать пасту из ресторана. В ассортименте много вариантов, но для некоторых уже нет ингредиентов.  Для болоньезе нужна томатная паста и мясной фарш, а для карбонары — сливочный соус и бекон.
...
#Задание 4
#Теперь друзьям надо решить, чем заняться в пятницу вечером. Важно, чтобы была подходящая погода.
#Заполни многоточия операторами или оставь пустыми так, чтобы условия описывали правильно каждое действие.
#Считай, что скейтпарк в помещении: можно кататься на скейтах, даже если плохая погода.
